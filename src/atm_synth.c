...
// NOTE: this block of progmem needs to stay since it cannot be read from the flash cart in ISR code
const uint16_t noteTable[64] PROGMEM = {0,    268,  284,  301,  319,  338,  358,  379,  401,  425,  451,  477,  506,  536,  568,  601,
                                        637,  675,  715,  758,  803,  851,  901,  955,  1011, 1072, 1135, 1203, 1274, 1350, 1430, 1515,
                                        1606, 1701, 1802, 1909, 2023, 2143, 2271, 2406, 2549, 2700, 2861, 3031, 3211, 3402, 3604, 3819,
                                        4046, 4286, 4541, 4811, 5098, 5401, 5722, 6062, 6422, 6804, 7209, 7638, 8092, 8573, 9083, 9623};
...
// TODO: shouldn't need any of these anymore
...
uint8_t incrementWriteIndex(uint8_t i) {
    if (++i == ATM_BUFFER_LENGTH) {
        return 0;
    }
    return i;
}

static void readIndexAdvance() {
    atmlib_state.readIndex = incrementWriteIndex(atmlib_state.readIndex);
}

static uint8_t nextReadIndex() {
    if (atmlib_state.readIndex + 1 > ATM_BUFFER_LENGTH) {
        return 0;
    }
    return atmlib_state.readIndex + 1;
}

static uint8_t readByteFromBuffer(uint8_t bufferIndex) {
    uint8_t val = atmlib_state.synthBuffer[bufferIndex][atmlib_state.readIndex];

    return val;
};
static uint16_t readWordFromBuffer(uint8_t bufferIndex) {
    uint8_t upper = atmlib_state.synthBuffer[bufferIndex][atmlib_state.readIndex];
    uint8_t index = nextReadIndex();
    uint8_t lower = atmlib_state.synthBuffer[bufferIndex][index];
    return (upper << 8) | lower;
};

uint8_t readerPos() {
    return atmlib_state.readIndex;
}

void link_channels(uint8_t *channelPointers[4]) {
    for (uint8_t i = 0; i < 4; i++)
        atmlib_state.synthBuffer[i] = channelPointers[i];
};
...
// TODO: prob remove all of this since entrys will be handled by the FX chip
static const uint8_t *get_entry_pattern_array_ptr(const uint8_t *score) {
    ...
    // TODO need to change this advance of the pointer
    return hdr.f.fmt & 0x01 ? score + sizeof(uint16_t) * pattern_count + 3 : NULL;
}

// TODO: shouldn't need this since track starts are handled in cpp namespace
...
static void atm_synth_init_channel(struct atm_channel_state *ch, struct osc_params *dst, struct atm_synth_state *player,
...
// TODO: the score start pointer isnt needed anymore
...
void atm_synth_play_score(const uint8_t *score) {
    ...
    for (unsigned n = 0; n < ARRAY_SIZE(channels); n++) {
        atm_synth_init_channel(&channels[n], channels[n].dst_osc_params, &atmlib_state, pgm_read_byte(&ep[n]));
    }
    /* Start playback */
...
void atm_synth_play_ext(const struct atm_synth_ext *synth_ext) {
    ...
    /* Fetch starting points for each track */
    for (unsigned n = 0; n < ARRAY_SIZE(channels); n++) {
        atm_synth_init_channel(&channels[n], channels[n].dst_osc_params, &atmlib_state, 0);
    }
    /* Start playback */
...
static void process_channel(const uint8_t ch_index, struct atm_synth_state *score_state, struct atm_channel_state *ch) {
    ...
    while (ch->delay == 0) {
        ...
        */
        // TODO: read 4 bytes from the buffer
